import { NoteData, StaffData, ScoreData } from '../types';
import { NoteDiff, StaffDiff, ScoreDiff, DiffType } from '../types/extended';
...
export class DiffUtils {
  ...
  getNoteDiffsByType(diff: ScoreDiff, type: DiffType): NoteDiff[] {
    return diff.notes.filter(d => d.type === type);
  }

  getStaffDiffsByType(diff: ScoreDiff, type: DiffType): StaffDiff[] {
    return diff.staves.filter(d => d.type === type);
  }

  isNoteModified(diff: NoteDiff, property: keyof NoteData): boolean {
    if (!diff.changes) return false;
    return diff.changes[property] !== undefined;
  }

  getNoteDiffColor(type: DiffType): string {
