#[cfg(feature = "server")]
mod server_main {
    pub use particle_simulation::server::main::main;
}
...
mod client {
    ...
    use particle_simulation::particle::{ParticleType, InteractionRules};
    use particle_simulation::simulation::{Simulation, SimulationConfig, BoundaryType};
    use particle_simulation::protocol::{
        ClientMessage, ServerMessage, ConnectionState, UserInfo, UserAction, ControlAction,
    };
            ...
            match msg {
                ...
                ServerMessage::StateDelta { added_particles, removed_particle_indices, particle_updates, config_update, is_running, timestamp } => {
                    if !self.is_host_mode {
                        ...
                        let mut indices = removed_particle_indices.clone();
